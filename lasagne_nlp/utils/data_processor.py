...
def read_conll_sequence_labeling(path, word_alphabet, label_alphabet, word_column=0, label_column=1):
    """
    ...
    print 'word_column: %d, label_column: %d' % (word_column, label_column)
    word_sentences = []
    label_sentences = []
    ...
    with open(path) as file:
        for line in file:
            ...
            if line == "":  # empty line => end of sentence
                ...
                word_ids = []
                label_ids = []
            elif line.split()[0][0]=='#' and word_column!=0:
                continue
            else:  # non-empty line
                tokens = line.split()
...
def generate_character_data(sentences_train, sentences_dev, sentences_test, max_sent_length, char_embedd_dim=30):
    ...
    return C_train, C_dev, C_test, build_char_embedd_table() # build_char_embedd_table() is a tuple
...
def build_embedd_table(word_alphabet, embedd_dict, embedd_dim, caseless):
    ...
    for word, index in word_alphabet.iteritems():
        ...
        if ww in embedd_dict:
            ...
            in_emb_vocab += 1
        elif ww.lower() in embedd_dict:
            embedd = embedd_dict[ww.lower()]
            in_emb_vocab += 1
        else:
            embedd = np.random.uniform(-scale, scale, [1, embedd_dim])
            out_emb_vocab += 1
        embedd_table[index, :] = embedd
    logger.info("#words found in embedd_dict: %d, not found in embedd_dict: %d" % (in_emb_vocab, out_emb_vocab))

    word_freqs = word_alphabet.get_vocab_freqs()
    return (embedd_table, word_freqs)


def load_dataset_sequence_labeling(train_path, dev_path, test_path, word_column=0, label_column=3,
    ...
    def generate_dataset_fine_tune():
        ...
        C_train, C_dev, C_test, (char_embedd_table, char_freqs) = generate_character_data(word_sentences_train, word_sentences_dev,
                                                                            word_sentences_test,
                                                                            max_length) if use_character else (
            None, None, None, None)
        return X_train, Y_train, mask_train, X_dev, Y_dev, mask_dev, X_test, Y_test, mask_test, \
               build_embedd_table(word_alphabet, embedd_dict, embedd_dim, caseless), label_alphabet, \
               C_train, C_dev, C_test, (char_embedd_table, char_freqs)
        # build_embedd_table() is a tuple
    ...
    def generate_dataset_not_fine_tune():
        ...
        return X_train, Y_train, mask_train, X_dev, Y_dev, mask_dev, X_test, Y_test, mask_test, \
               None, label_alphabet, C_train, C_dev, C_test, (char_embedd_table, char_freqs)
